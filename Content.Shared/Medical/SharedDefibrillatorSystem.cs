...
public abstract class SharedDefibrillatorSystem : EntitySystem
{
    ...
    public bool CanZap(Entity<DefibrillatorComponent?> ent, EntityUid target, EntityUid? user = null, bool targetCanBeAlive = false)
    {
        ...
        if (!_toggle.IsActivated(ent.Owner) && !ent.Comp.IgnoreToggle) // imp ignoretoggle
        {
        ...
        if (!_powerCell.HasActivatableCharge(ent.Owner, user: user, predicted: true) && !ent.Comp.IgnorePowerCell) // imp ignorepowercell
            return false;
        ...
        // imp
        return _whitelist.IsWhitelistPassOrNull(ent.Comp.Whitelist, target);
    }
    ...
    public bool TryStartZap(Entity<DefibrillatorComponent?> ent, EntityUid target, EntityUid user)
    {
        ...
        if (ent.Comp.SkipDoAfter) // imp add, multitool defib
        {
            Zap(ent, target, user);
            return false;
        }

        if (ent.Comp.PlayChargeSound) // imp if
            _audio.PlayPredicted(ent.Comp.ChargeSound, ent.Owner, user);
        return _doAfter.TryStartDoAfter(
            new DoAfterArgs(EntityManager, user, TimeSpan.FromSeconds(ent.Comp.DoAfterDuration), new DefibrillatorZapDoAfterEvent(),
            ent.Owner, target, ent.Owner) // imp doafter -> fromseconds
            {
                NeedHand = true,
    ...
    public void Zap(Entity<DefibrillatorComponent?> ent, EntityUid target, EntityUid user)
    {
        ...
        if (!_powerCell.TryUseActivatableCharge(ent.Owner, user: user) && !ent.Comp.IgnorePowerCell) //imp, check IgnorePowerCell
            return;
        ...
        if (ent.Comp.PlayZapSound) // imp if
            _audio.PlayPredicted(ent.Comp.ZapSound, ent.Owner, user);
        _electrocution.TryDoElectrocution(target, ent.Owner, ent.Comp.ZapDamage, TimeSpan.FromSeconds(ent.Comp.WritheDuration), true, ignoreInsulation: true); // imp timespan
        ...
        foreach (var other in _interacters)
        {
            ...
            _electrocution.TryDoElectrocution(other, null, ent.Comp.ZapDamage, TimeSpan.FromSeconds(ent.Comp.WritheDuration), true); // imp timespan
        }
