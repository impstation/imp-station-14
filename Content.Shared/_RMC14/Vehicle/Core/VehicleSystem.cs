...
public sealed class VehicleSystem : EntitySystem
{
    ...
    // imp figure out way to not do this, probably just rename
    ...
    private bool TryFindEntry(Entity<VehicleEnterComponent> ent, EntityUid user, out int entryIndex)
    {
        ...
        var worldRotation = _transform.GetWorldRotation(vehicleXform); // imp
        var localDelta = (-worldRotation).RotateVec(delta); // imp use world rotation instead of vehicle xform local rotation
    ...
    private void UntrackOccupant(EntityUid user, EntityUid vehicle)
    {
        if (!TryComp(user, out VehicleInteriorOccupantComponent? occupant) ||
            occupant.Vehicle != vehicle)
        {
            UnregisterTrackedOccupant(vehicle, user, false);
