...
/// <summary>
/// System for handling the WeaponRecall ability from the recall implant. This is pretty much entirely copied from the wizard spell
/// </summary>
public abstract partial class SharedWeaponRecallSystem : EntitySystem
{
    ...
    private void RecallItem(Entity<WeaponRecallMarkerComponent?> ent)
    {
        ...
        _popups.PopupPredicted(Loc.GetString("weapon-recall-item-summon-self", ("item", ent)),
                               Loc.GetString("weapon-recall-item-summon-others", ("item", ent), ("name", Identity.Entity(actionOwner.Value, EntityManager))),
                               actionOwner.Value, actionOwner.Value);
        _popups.PopupPredictedCoordinates(Loc.GetString("weapon-recall-item-disappear", ("item", ent)), Transform(ent).Coordinates, actionOwner.Value);
    ...
    private void TryUnmarkItem(EntityUid item)
    {
        ...
        if (TryComp<WeaponRecallComponent>(marker.MarkedByAction, out var action))
        {
            ...
            {
                _popups.PopupClient(Loc.GetString("weapon-recall-item-unmark", ("item", item)), instantAction.AttachedEntity.Value, instantAction.AttachedEntity.Value, PopupType.MediumCaution);
    ...
    private void UpdateActionAppearance(Entity<WeaponRecallComponent> action)
    {
