...
public sealed class SlasherMeatHookSystem : EntitySystem
{
    ...
    private void OnPlaceMeathook(SlasherPlaceMeathookEvent args)
    {
        ...
        var minHookDistance = hookConfig.MinimumHookDistance;
        if (IsTooCloseToExistingHook(spawnCoords, minHookDistance))
        {
            _popup.PopupEntity(Loc.GetString("slasher-meathook-place-too-close-hook", (DistanceLocArg, minHookDistance)),
                args.Performer,
                args.Performer,
                PopupType.MediumCaution);
            return;
    ...
    private bool CanStartHarvest(EntityUid user, out HandsComponent? hands)
    {
        ...
        return _hands.GetEmptyHandCount((user, hands)) >= 2;
    }
    ...
    private bool IsTooCloseToExistingHook(EntityCoordinates spawnCoords, float minimumHookDistance)
    {
        var hookQuery = EntityQueryEnumerator<SlasherMeatHookComponent, TransformComponent>();
        while (hookQuery.MoveNext(out _, out _, out var hookXform))
        {
            if (!hookXform.Coordinates.TryDistance(EntityManager, spawnCoords, out var distance))
                continue;

            if (distance < minimumHookDistance)
                return true;
        }
