...
public sealed partial class SupermatterSystem
{
    ...
    private void ProcessAtmos(EntityUid uid, SupermatterComponent sm, float frameTime)
    {
        ...
        sm.MoleHeatPenaltyThreshold = (float)Math.Max(sm.GasStorage.TotalMoles / _config.GetCVar(ImpCCVars.SupermatterMoleHeatPenalty), 0.25);

        // Ramps up or down in increments of 0.02 up to the proportion of CO2
        // Given infinite time, powerloss_dynamic_scaling = co2comp
        // Some value from 0-1
        if (sm.GasStorage.TotalMoles > _config.GetCVar(ImpCCVars.SupermatterPowerlossInhibitionMoleThreshold) && // if there are more than 20 mols,
            sm.GasComposition.GetMoles(Gas.CarbonDioxide) > _config.GetCVar(ImpCCVars.SupermatterPowerlossInhibitionGasThreshold)) // and more than 20% co2
        {
        ...
        sm.PowerlossInhibitor = Math.Clamp(
            1 - sm.PowerlossDynamicScaling * Math.Clamp(sm.GasStorage.TotalMoles / _config.GetCVar(ImpCCVars.SupermatterPowerlossInhibitionMoleBoostThreshold), 1f, 1.5f),
            0f, 1f);
    ...
    private void HandleDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.GasStorage is { })
        {
            ...
            var moleDamage = Math.Max(sm.GasStorage.TotalMoles - _config.GetCVar(ImpCCVars.SupermatterMolePenaltyThreshold), 0f) / 80 * sm.DamageIncreaseMultiplier;
            totalDamage += moleDamage;
        }

        // Healing damage
        if (sm.GasMixture is { } && sm.GasStorage is { } && sm.GasStorage.TotalMoles < _config.GetCVar(ImpCCVars.SupermatterMolePenaltyThreshold))
        {
    ...
    private void AnnounceCoreDamage(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.Damage >= sm.DamageWarningThreshold)
        {
            ...
            if (sm.GasStorage is { } && sm.GasStorage.TotalMoles >= _config.GetCVar(ImpCCVars.SupermatterMolePenaltyThreshold))
            {
    ...
    public DelamType ChooseDelamType(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (sm.GasStorage is { })
        {
            if (_config.GetCVar(ImpCCVars.SupermatterDoSingulooseDelam)
                && sm.GasStorage.TotalMoles >= _config.GetCVar(ImpCCVars.SupermatterMolePenaltyThreshold) * _config.GetCVar(ImpCCVars.SupermatterSingulooseMolesModifier))
