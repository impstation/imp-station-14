...
public sealed partial class SupermatterSystem
{
    ...
    private void ProcessAtmos(EntityUid uid, SupermatterComponent sm, float frameTime)
    {
        ...
        if (!sm.Surging) // Imp change, if surging then dosen't calculate heat modifier and uses the currently set one
        ...
        if (!sm.Surging) // Imp change, if surging then it dosen't calculate power, instead using the currently set one
        ...
        if (!sm.Surging) // Imp change, if surging dosen't decay power so that lightning can appear
