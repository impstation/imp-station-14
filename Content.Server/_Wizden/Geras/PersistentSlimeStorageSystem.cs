...
/// <summary>
/// Uses for transfering items between forms that share persistentSlimeStorage components.
/// Also shows items contained in storages on examine
/// </summary>
public sealed class PersistentSlimeStorageSystem : EntitySystem
{
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<PersistentSlimeStorageComponent, PolymorphedEvent>(OnPolymorph);
        SubscribeLocalEvent<PersistentSlimeStorageComponent, ExaminedEvent>(OnExamine);
    }
    ...
    private void OnExamine(Entity<PersistentSlimeStorageComponent> ent, ref ExaminedEvent args)
    {
        ...
        using (args.PushGroup(nameof(PersistentSlimeStorageComponent)))
        {
    ...
    /// <param name="ent"></param>
    /// <param name="args"></param>
    private void OnPolymorph(Entity<PersistentSlimeStorageComponent> ent, ref PolymorphedEvent args)
    {
        if (!_entityManager.TryGetComponent<StorageComponent>(ent, out var storage))
            return;
        if (!HasComp<PersistentSlimeStorageComponent>(args.NewEntity) || !_entityManager.HasComponent<StorageComponent>(args.NewEntity))
        {
