...
public sealed partial class ChatSystem : SharedChatSystem
{
    ...
    public readonly Color DefaultSpeakColor = Color.LightGray;
    public readonly Color DefaultWhisperColor = Color.DarkGray;
        ...
        )
    {
        ...
        if ((_adminManager.IsAdmin(player) && _adminManager.HasAdminFlag(player, AdminFlags.Moderator)) // Override if admin
            || !HasComp<GhostBarPatronComponent>(source) // I'm part of the problem!
            || _deadLoocEnabled
        ...
        )
    {
        ...
        var languageObfuscatedMessage = SanitizeInGameICMessage(source,
            ...
            (!CultureInfo.CurrentCulture.IsNeutralCulture && CultureInfo.CurrentCulture.Parent.Name == "en") || (CultureInfo.CurrentCulture.IsNeutralCulture && CultureInfo.CurrentCulture.Name == "en"));
    ...
    public string WrapMessage(LocId wrapId, InGameICChatType chatType, EntityUid source, string entityName, string message, LanguagePrototype? language)
    {
        ...
        if (chatType == InGameICChatType.Whisper) //kinda messy but eh that doesn't really deserve a full switch statement
            color = DefaultWhisperColor;
